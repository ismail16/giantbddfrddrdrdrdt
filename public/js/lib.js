    ...
    if (typeof l != "undefined") {
        ...
        if (i.expires && (typeof i.expires == "number" || i.expires.toUTCString)) {
            if (typeof i.expires == "number") {
                k = new Date;
                k.setTime(k.getTime() + i.expires * 24 * 60 * 60 * 1E3)
            } else k = i.expires;
            k = "; expires=" + k.toUTCString()
        }
        var q = i.path ? "; path=" + i.path : "", u = i.domain ? "; domain=" + i.domain : "";
        i = i.secure ? "; secure" : "";
        document.cookie = [h, "=", encodeURIComponent(l), k, q, u, i].join("")
    } else {
        l = null;
        if (document.cookie && document.cookie != "") {
            ...
            for (k = 0; k < i.length; k++) {
                q = jQuery.trim(i[k]);
                if (q.substring(0, h.length + 1) == h + "=") {
                    l = decodeURIComponent(q.substring(h.length + 1));
                    break
                }
            }
        }
...
(function (h) {
    h.fn.tableFilter = function (l) {
        ...
        function k() {
            return m.attr("id") || m.attr("name")
        }

        function q() {
            n.filter("input").keyup(r);
            n.filter("select").change(r);
            if (g.clearFiltersControls) for (var a = 0; a < g.clearFiltersControls.length; a++) g.clearFiltersControls[a].click(function () {
                u();
                return false
            });
            if (g.additionalFilterTriggers) for (a = 0; a < g.additionalFilterTriggers.length; a++) {
                ...
                switch (b.attr("type")) {
                    ...
                    case "checkbox":
                        ...
                        break;
                    default:
                        throw"Filter type " + b.attr("type") + " is not supported";
                }
            }
        }
        ...
        function u() {
            n.val("");
            if (g.additionalFilterTriggers) for (var a = 0; a < g.additionalFilterTriggers.length; a++) {
                var b = g.additionalFilterTriggers[a];
                switch (b.attr("type")) {
                    case "text":
                        b.val("");
        ...
        function H() {
            w = m.find("thead tr:first th");
            o = m.find("tbody tr");
            J();
            n = m.find("thead tr:last").find("input,select");
            z = [];
            n.each(function () {
                z.push(K($(this)))
            })
        ...
        function J() {
            for (var a = $("<tr class='filters'></tr>"), b = 0; b < w.length; b++) {
                var c = $(w[b]), d = c.attr("filter") === "false" ? "" : c.text();
                if (d.length > 1) {
                    d = $("<td/>");
                    var e = $(L(b, c));
                    e.width(c.width() - 10);
        ...
        function L(a, b) {
            ...
            switch (c) {
                case "text":
                    return "<input type='text' id='filter_" + a + "' class='filter' title='" + h.fn.tableFilter.filterToolTipMessage + "'/>";
                case "ddl":
                    return M(a, b);
                default:
        ...
        function M(a) {
            var b = "<select id='filter_" + a + "' class='filter'><option value=''>" + g.selectOptionLabel + "</option>";
            a = o.find("td:nth-child(" + (a + 1) + ")");
            var c = [];
            h.each(a, function () {
                var d = $(this).text();
                if (!(!d || d === "&nbsp;")) {
                    for (var e = 0; e < c.length; e++) if (c[e] === d) return;
                    c.push(d)
                }
            });
            c.sort();
            h.each(c, function () {
                b += '<option value="' + this.replace('"', "&#034;") + '">' + this + "</option>"
            });
            b += "</select>";
            return b
        }
            ...
            if (h.cookie) {
                ...
                if (a) {
                    ...
                    for (var b = 0; b < a.length; b++) {
                        var c = a[b].split(",");
                        a[b] = {id: c[0], value: c[3], idx: c[1], type: c[2]}
                    }
                    A(a, true)
                }
            }
        ...
        function C() {
            ...
            if (a - B >= g.filterDelay) y(); else s = setTimeout(C, g.filterDelay / 3)
        }
        ...
        function N() {
            ...
            if (!g.additionalFilterTriggers) return a;
            for (b = 0; b < g.additionalFilterTriggers.length; b++) if (c = D(g.additionalFilterTriggers[b])) a[a.length] = c;
            return a
        }
        ...
        function D(a) {
            ...
            var c = P(a);
            return {id: a.attr("id"), value: b.toString(), idx: c, type: a.attr("type")}
        }
        ...
        function Q(a, b) {
            R();
            if (!((!a || a.length) === 0 && (g.matchingRow === null || g.matchingCell))) if (a === null || a.length === 0) E(null); else for (var c = 0; c < a.length; c++) {
                var d = a[c];
                if (b && d.type && d.id) switch (d.type) {
                    case "select-one":
                    case "text":
                        m.find("#" + d.id).val(d.value);
                        break;
                    case "checkbox":
                        m.find("#" + d.id).attr("checked", d.value === "true");
                        break;
                    default:
                        throw"Filter type " + d.type + " is not supported";
                }
                E(d)
            }
            S()
        ...
        function E(a) {
            for (var b = T(a), c = a === null ? -1 : a.idx, d = 0; d < o.length; d++) {
                if (t) return;
                var e = h(o[d]);
                e.attr("filtermatch") || U(a, e, b, c) || e.attr("filtermatch", "false")
            }
        }
        ...
        function U(a, b, c, d) {
            var e = b.children("td");
            return d < 0 ? F(a, b.text(), c) && G(a, b, c) : F(a, h(e[d]).text(), c) && G(a, b, c)
        }

        function G(a, b, c) {
            if (!g.matchingRow) return true;
            return g.matchingRow(a, b, c)
        }

        function F(a, b, c) {
            if (!W(b, c, a != null && a.type === "select-one")) return false;
            return !g.matchingCell || g.matchingCell(a, td, c)
        }
        ...
        function X(a) {
            ...
            for (var b = [], c = 0; c < a.length; c++) {
                for (var d = a[c], e = d.indexOf("("); e != -1;) {
                    if (e > 0) b[b.length] = d.substring(0, e);
                    b[b.length] = "(";
                    d = d.substring(e + 1);
                    e = d.indexOf("(")
                }
        ...
        function aa(a) {
            var b = /([^"^\s]+)\s*|"([^"]+)"\s*/g;
            a = a.match(b);
            for (b = 0; b < a.length; b++) a[b] = ba(a[b]).replace(/"/g, "");
            return a
        }
        ...
        function Y(a) {
            for (var b = [], c, d = 0; d < a.length; d++) {
                ...
                if (!(!e || e.length === 0)) {
                    ...
                    if (c) if (c != "(" && c != "not" && c != "and" && c != "or" && e != "and" && e != "or" && e != ")") b[b.length] = "and";
                    c = b[b.length] = e
                }
            }
            return b
        ...
        function Z(a) {
            for (var b = "", c = [], d, e = 0; e < a.length; e++) {
                var j = a[e];
                if (j.length !== 0) if (j != "and" && j != "or" && j != "not" && j != "(" && j != ")") b = b + "|" + j; else if (c.length === 0 || j === "(") c.push(j); else if (j === ")") for (d = c.pop(); d != "(";) {
                    b = b + "|" + d;
                    d = c.pop()
                } else {
                    if (c[c.length - 1] !== "(") for (; c.length != 0;) {
        ...
        function W(a, b, c) {
            if (!b) return true;
            a = c ? a : a.toLowerCase();
            for (var d = [], e, j, x = 0; x < b.length; x++) {
                token = b[x];
                if (token != "and" && token != "or" && token != "not") d.push(c ? a === token : a.indexOf(token) >= 0); else if (token === "and") {
                    e = d.pop();
                    j = d.pop();
                    d.push(e && j)
                } else if (token === "or") {
        ...
        var B, s, m, t, v, n, z, w, o, g = l || h.fn.tableFilter.defaults;
        if (!g.filterDelay) g.filterDelay = h.fn.tableFilter.defaults.filterDelay;
        if (!g.selectOptionLabel) g.selectOptionLabel = h.fn.tableFilter.defaults.selectOptionLabel;
        this.each(function () {
            m = h(this);
            i()
        });
...
(function ($) {
    ...
    m.defaults = {axis: 'xy', duration: parseFloat($.fn.jquery) >= 1.3 ? 0 : 1};
    m.window = function (b) {
    ...
    $.fn.scrollTo = function (l, j, a) {
        ...
        if (typeof a == 'function') a = {onAfter: a};
        if (l == 'max') l = 9e9;
        ...
        return this.scrollable().each(function () {
            ...
            $.each(a.axis.split(''), function (b, h) {
                var f = h == 'x' ? 'Left' : 'Top', i = f.toLowerCase(), c = 'scroll' + f, r = k[c],
                    s = h == 'x' ? 'Width' : 'Height';
                if (p) {
...
function mdpiComAdserverRotateBanner(div) {
    ...
    if (ifvisible.now()) {
        if (mdpi_com_adserver_banners[id]["fetchingActive"]) {
            $.ajax({
                url: url, type: 'GET', crossDomain: true, contentType: 'text/plain', success: function (data) {
                    if (data.data == false) {
                        ...
                        if (matches) {
                            ...
                            if (alreadyLoaded) {
                                ...
                                mdpi_com_adserver_banners[id]["loadedBanners"].push({
                                    'id': matches[2],
                                    'content': content
                                });
                                mdpiComAdserverRotateImg(div);
...
try {
    ...
    function _jmolGetJarFilename(fileNameOrFlag) {
        _jmol.archivePath = (typeof (fileNameOrFlag) == "string" ? fileNameOrFlag : (fileNameOrFlag ? "JmolAppletSigned" : "JmolApplet") + "0.jar");
    }
    ...
    function jmolCheckbox(scriptWhenChecked, scriptWhenUnchecked, labelHtml, isChecked, id, title) {
        ...
        ++_jmol.checkboxCount;
        if (scriptWhenChecked == undefined || scriptWhenChecked == null || scriptWhenUnchecked == undefined || scriptWhenUnchecked == null) {
    ...
    function jmolAppletInline(size, inlineModel, script, nameSuffix) {
        _jmolInitCheck();
        return _jmolApplet(size, _jmolSterilizeInline(inlineModel), script, nameSuffix);
    }
    ...
    function jmolCheckBrowser(action, urlOrMessage, nowOrLater) {
        ...
        if (typeof action != "string")
            alert("jmolCheckBrowser(action, urlOrMessage, nowOrLater)\n\n" +
                "action must be 'alert', 'redirect', or 'popup'"); else {
            if (typeof urlOrMessage != "string")
                alert("jmolCheckBrowser(action, urlOrMessage, nowOrLater)\n\n" +
                    "urlOrMessage must be a string"); else {
    ...
    var _jmol = {
        ...
        windowsCabUrl: "http://java.sun.com/update/1.5.0/jinstall-1_5_0_05-windows-i586.cab",
    ...
    with (_jmol) {
        ...
        _jmolTestUA("konqueror") || _jmolTestUA("safari") || _jmolTestUA("omniweb") || _jmolTestUA("opera") || _jmolTestUA("webtv") || _jmolTestUA("icab") || _jmolTestUA("msie") || (_jmol.ua.indexOf("compatible") < 0 && _jmolTestUA("mozilla"));
        _jmolTestOS("linux") || _jmolTestOS("unix") || _jmolTestOS("mac") || _jmolTestOS("win");
        isNetscape47Win = (os == "win" && browser == "mozilla" && browserVersion >= 4.78 && browserVersion <= 4.8);
        if (os == "win") {
            ...
            if (browser == "mozilla" && browserVersion >= 5) {
                ...
                isBrowserCompliant = hasGetElementById && !((browser == "msie") || (browser == "safari" && browserVersion < 125.12));
            }
        ...
        useIEObject = isIEWin;
        useHtml4Object = (os != "mac" && browser == "mozilla" && browserVersion >= 5) || (os == "win" && browser == "opera" && browserVersion >= 8) || (os == "mac" && browser == "safari" && browserVersion >= 412.2);
        doTranslate = true;
    ...
    if (noJavaMsg2 == undefined) var noJavaMsg2 = "You do not have the<br />\n" +
    ...
    function _jmolApplet(size, inlineModel, script, nameSuffix) {
        with (_jmol) {
            ...
            if (useIEObject) {
                ...
                tHeader = "<object name='jmolApplet" + nameSuffix +
                    "' id='jmolApplet" + nameSuffix + "' " + appletCssText + "\n" +
                    " classid='" + windowsClassId + "'\n" + winCodebase + widthAndHeight + ">\n";
                tFooter = "</object>";
            } else if (useHtml4Object) {
                tHeader = "<object name='jmolApplet" + nameSuffix +
                    "' id='jmolApplet" + nameSuffix + "' " + appletCssText + "\n" +
                ...
                tHeader = "<applet name='jmolApplet" + nameSuffix +
    ...
    function _jmolCheckBrowser() {
        with (_jmol) {
            ...
            if (checkBrowserAction == "redirect")
                ...
                _jmolPopup(checkBrowserUrlOrMessage); else {
    ...
    function _jmolGetDomElement(data, Ptr, closetag, lvel) {
        ...
        var e = (tagname == closetag || tagname == "/" ? "" : document.createElementNS ? document.createElementNS('http://www.w3.org/1999/xhtml', tagname) : document.createElement(tagname));
        if (ch == ">") {
            Ptr[0] = ++pt;
            return e
        }
    ...
    function _jmolPopup(url) {
        var popup = window.open(url, "JmolPopup", "left=150,top=150,height=400,width=600," +
    ...
    function _jmolFindApplet(target) {
        ...
        if (applet == undefined)
            applet = _jmolSearchFrames(top, target);
        return applet;
    }
...
if (document.location.search.indexOf("NOAPPLET") >= 0) {
    ...
    jmolApplet0 = {
        script: function (script) {
            document.getElementById("fakeApplet").value = "\njmolScript:\n" + script
        }, scriptWait: function (script) {
            document.getElementById("fakeApplet").value = "\njmolScriptWait:\n" + script
        }, loadInline: function (data, script) {
            document.getElementById("fakeApplet").value = "\njmolLoadInline data:\n" + data + "\n\nscript:\n" + script
        }
    }
}
...
function _jmolFixDim(x, units) {
    var sx = "" + x;
    return (sx.length == 0 ? (units ? "" : _jmol.allowedJmolSize[2]) : sx.indexOf("%") == sx.length - 1 ? sx : (x = parseFloat(x)) <= 1 && x > 0 ? x * 100 + "%" : (isNaN(x = Math.floor(x)) ? _jmol.allowedJmolSize[2] : x < _jmol.allowedJmolSize[0] ? _jmol.allowedJmolSize[0] : x > _jmol.allowedJmolSize[1] ? _jmol.allowedJmolSize[1] : x) + (units ? units : ""));
}
...
(function () {
    $(function () {
        ...
        $('.tabbing').map(function (index) {
            ...
            tab1.children('ul').children('li').map(function (index) {
                ...
                if (index > 3) {
                    ...
                    return $.ajax({
                        url: tmp_url + "?ajax_preview=count", success: function (data) {
        ...
        $('.tabbing > ul > *').bind("click", function () {
            ...
            $.ajax({
                url: selected.children('a').attr('href'), success: function (data) {
...
var globalAmount = $("#payAmount").val();
var globalAmountCHF = $("#payAmount").val();
var globalCurrency = 'CHF';
(function () {
    $(function () {
        var counter, counter_amount, doUpdate, hasInvoice, label, originalAmountCHF, originalCurrency, showAlert,
            updatePayment;
        counter = 0;
        counter_amount = 0;
        if ($('#payment').size() === 0) {
        ...
        updatePayment = function (radio) {
            ...
            var hasPostSelectedInvoice = $('#currency_chf').next('label').data('has-post-selected-invoice');
            unit = label.html();
            if (hasInvoice) {
                exchange = radio.next('label').data('apc-exchange-rate');
                amount = globalAmountCHF;
            } else if (hasPostSelectedInvoice) {
                exchange = radio.next('label').data('apc-exchange-rate');
                amount = globalAmountCHF;
            } else {
                exchange = 1;
        ...
        return $('#payment').on('submit', function (e) {
            ...
            if (validated) {
                ...
                var form = this;
                $.ajax({
                    url: url, dataType: 'json', success: function (results) {
                        if (results.length == 0) {
                            form.submit();
                            return false;
                        }
                        englishChyInvoice = function () {
                            $html = "<div><h6>Is not allowed to pay invoices with CNY currency on payment.mdpi.com<br>";
                            $('<div></div>').appendTo('body').html($html).dialog({
                                modal: true,
                                title: 'This invoice is in CNY ',
                                zIndex: 10000,
                                autoOpen: true,
                                width: '500px',
                                resizable: false,
                        ...
                        if (results[0].value.substring(0, 8) == "english-" && results[0].currency == "CNY") {
                            englishChyInvoice();
                        } else {
...
(function () {
    $(function () {
        var all_option, current_url, div_my_query_history, div_save_my_query, empty_option, fill_options,
            journal_changed, select_option, showHistoryQuery, showSaveQuery, update_sections, update_special_issues;
        ...
        update_sections = function (journal_name) {
            if (journal_name === '') {
                $('#section').html(empty_option).trigger("chosen:updated");
                $('#special_issue').html(empty_option).trigger("chosen:updated");
                return;
            }
        ...
        $('#add-search-url').click(function (e) {
            ...
            if (description === '') {
                ...
                $(this).val('saving');
                return $.get($(this).data('saveurl-url'), {url_desc: description, url: current_url}, function (res) {
                    var delete_img;
        ...
        showHistoryQuery = function () {
            $('#my-query-history').css({"display": "inline-block"});
...
$(function () {
    $("#refno").on('input propertychange', function (e) {
            ...
            $.ajax({
                url: url, dataType: 'json', success: function (results) {
                    ...
                    $('<div></div>').appendTo('body').html($html).dialog({
                        ...
                        buttons: {
                            Yes: function () {
                                var value = $("input:radio[name=invoice]:checked").val();
                                if (results[value].currency == "CNY") {
                                    alert("The selected invoice is in CNY, this currency is currently not accepted for online payment,\nplease contact our Billing department at billing-cn@mdpi.com for additional information");
                                    $(this).dialog("close");
                                    $('#refno').val("");
                                    $('#payAmount').val(0);
                                    return;
                                ...
                                globalAmount = results[value].amount;
                                globalCurrency = results[value].currency;
                                var exchangeRate = $('#currency_' + globalCurrency.toLowerCase()).next('label').data('apc-exchange-rate');
                                globalAmountCHF = parseFloat(globalAmount / exchangeRate).toFixed(2);
                                $('#payAmount').prop('readonly', true);
                                $('#currency_' + results[value].currency.toLowerCase()).trigger("click");
                                ...
                                $('#currency_chf').next('label').data("has-post-selected-invoice", "true");
                                $('#currency_chf').next('label').data('amount', total);
                                $(this).dialog("close");
                            }, No: function () {
                                $(this).dialog("close");
                                $('#refno').val("");
...
(function () {
    $(function () {
        ...
        var getDuration = function (grammar, layout, plagiarism, level) {
            var m = moment();
            if (layout) {
                m = momentAddDays(m, 1, false);
                result = "Editing will be completed on " + m.format("D MMMM");
            }
            if (plagiarism) {
                m = momentAddDays(m, 1, false);
                result = "Editing will be completed on " + m.format("D MMMM");
            ...
            if (grammar) {
                switch (level) {
                    case "regular":
                        m = momentAddDays(m, 2, false);
                        result = "Editing will be completed on " + m.format("D MMMM");
                        break;
                    case "express":
                        m = momentAddDays(m, 1, true);
                        result = "Editing will be completed on " + m.format("D MMMM HH:mm");
                        break;
                    case "specialist":
                        m = momentAddDays(m, 4, false);
                        result = "Editing will be completed on " + m.format("D MMMM");
                        break
                }
            }
        ...
        var get_price = function (word_num, has_manuscript, grammar, layout, plagiarism, level) {
            var price = 0;
            if (grammar) {
                switch (level) {
                    case "regular":
                        price += 105 + word_num * 0.02;
                        break;
                    case "express":
                        price += 175 + word_num * 0.025;
                        break;
                    case "specialist":
                        price += 180 + word_num * 0.04;
                        break
                }
            }
            if (layout) {
        ...
        var getEncodedParamsString = function (word_num, grammar, layout, plagiarism, level, subject) {
        ...
        var updateSubmitLink = function (submit) {
            ...
            var id = $('#manuscript_id').val().match(/\d+/);
            var level = null;
            var subject = null;
            var grammar = $("#service-grammar").is(":checked");
            var layout = $("#service-layout").is(":checked");
            var plagiarism = $("#service-plagiarism").is(":checked");
            if (id) {
                id = id[0];
            }
            if (grammar) {
            ...
            var manuscript_id = parseInt(id, 10);
            var susy_host = $('#susy_host').val();
            if (word_num > 0 && (grammar || layout || plagiarism)) {
                if (manuscript_id > 0) {
                    $.get('/is_under_review/' + manuscript_id, function (data) {
                        var has_manuscript = data.exist;
                        var price = get_price(word_num, has_manuscript, grammar, layout, plagiarism, level);
                        $("#price").html(price);
                        $("#duration").html(getDuration(grammar, layout, plagiarism, level));
                        var params = getEncodedParamsString(word_num, grammar, layout, plagiarism, level, subject);
                        if (!has_manuscript) {
                            $('#msg').html('No manuscript found!').show();
                            $('#proceed_btn').data('href', susy_host + 'user/pre_english/upload' + "?params=" + params);
                        } else {
                            $('#msg').html('').hide();
                            $('#proceed_btn').data('href', susy_host + 'user/manuscripts/pre_english_editing/' + data.hash_key + "?params=" + params);
                        }
                        if (submit) {
                    ...
                    var price = get_price(word_num, false, grammar, layout, plagiarism, level);
                    $("#price").html(price);
                    $("#duration").html(getDuration(grammar, layout, plagiarism, level));
                    var params = getEncodedParamsString(word_num, grammar, layout, plagiarism, level, subject);
                    $('#proceed_btn').data('href', susy_host + 'user/pre_english/upload' + "?params=" + params);
...
(function () {
    ...
    AbstractChosen = (function () {
        ...
        AbstractChosen.prototype.winnow_results = function (options) {
            var escapedQuery, fix, i, len, option, prefix, query, ref, regex, results, results_group, search_match,
                startpos, suffix, text;
    ...
    Chosen = (function (superClass) {
        ...
        Chosen.prototype.set_up_html = function () {
            ...
            container_props = {'class': container_classes.join(' '), 'title': this.form_field.title};
        ...
        Chosen.prototype.search_field_scale = function () {
            ...
            style_block = {position: 'absolute', left: '-1000px', top: '-1000px', display: 'none', whiteSpace: 'pre'};
