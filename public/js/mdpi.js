...
var TIMEOUT = 200, EVENT_KEY = 'resizeend', $window = $(window), timer;
...
$(document).ready(function () {
    ...
    $(".extending-content").each(function () {
        ...
        if (element.data('url')) {
            $.ajax({
                url: element.data('url'), success: function (msg) {
    ...
    $("#captchaModal").on("submit", "#emailCaptchaForm", function (e) {
        ...
        $.post(form.attr('action') + params, form.serialize(), function (data) {
            if (data.succ) {
                ...
                confirm.load("/email/captcha_confirmation", {
                    email: param,
                    type: type,
                    _token: data.csrf
                }, function (data) {
    ...
    $('.accessCaptcha').click(function (e) {
        ...
        $.post('/access/captcha/check/' + target, function (data) {
            if (data.succ) {
    ...
    $(document).on("click", '.dwnld_block .openpopupgallery', function () {
        var target = $(this).data('target');
...
function handleJournalActiveBasedLinks(journalName, link, event, isPublic, isAcceptingSubmissions, isDiscontinued) {
    ...
    if (isDiscontinued) {
        ...
        msg = "<em>" + journalName + "</em> is not public: " + link.data("disabledmessage");
